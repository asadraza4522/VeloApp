...
>
> **FFmpeg pulled forward, partially done (2026-09-23):** VP9/AV1-video mux fallback shipped (real bug found via user testing — Instagram Reels serve VP9-only video, which MediaMuxer's MP4 writer can't mux; `quality.ts`/`job.ts` now route that case to a `mux-ffmpeg` postProcess → `FfmpegRunner.kt`, a subprocess wrapping `io.github.junkfood02.youtubedl-android:ffmpeg` — GPLv3, same artifact the "Seal" app ships, stream-copies into Matroska, no re-encode). This needed a `runCancellable`-style signature change (`Downloader.kt`'s `runCancellable` and `DownloadWorker.kt`'s `execute()` both had to become properly `suspend` to call the new suspend `mergeToMkv`), and a required Settings > About source-code-offer entry (GPL §6, not decorative — see `AboutSection` in `screens/settings/index.tsx`). **Needs on-device verification and a native rebuild** (`npx expo run:android`) before it does anything — this is a new native dependency, JS/Metro reload alone won't pick it up, and the exact extracted-binary path inside `FfmpegRunner.kt` could not be tested against a real device in the environment this was built in. TikTok's separate, unrelated problem (anti-bot session continuity, not a codec issue) is still open — see plan §R7 and `memory/tiktok-needs-device-resolve.md`.
>
