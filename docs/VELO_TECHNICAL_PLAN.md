...
| **9 FFmpeg conversion** ⚠️ **pulled forward, partially done (2026-09-23)**: VP9/AV1 mux fallback shipped (see below); MP3/OPUS conversion + `AudioConverter` interface not started | `velo-ffmpeg` module behind `AudioConverter`, MP3/OPUS, video→audio, HLS merge; **candidate:** parallel segmented downloads in `Downloader.kt` (N concurrent `Range` connections per file instead of today's sequential bounded chunks, each writing its own byte offset via the existing `RandomAccessFile`, no merge step, see R10) | MP3/OPUS convert on device; size measured; if parallel downloads land, throughput on a fast unthrottled host measurably beats sequential chunking |
...
- **R7 — IP-bound direct URLs (confirmed in Phase 2).** Real YouTube resolves return URLs with `ip=` (flagged `ip_bound: true`); a server-resolved URL may 403 on the phone. Measure in Phase 2 canaries; mitigations: prefer clients/formats that are not IP-bound, transparent re-resolve, and (Full build only) device-side resolving with `youtubedl-android`.
  - **Confirmed for TikTok specifically, 2026-09-23.** TikTok's CDN URL (`tk=tt_chain_token` in the query string, an Akamai-backed anti-bot session token) needs continuity with the session/cookies that extracted it. Our split architecture (worker resolves, phone downloads — a different machine/TLS session/IP than whatever did the extraction) breaks this even when headers match exactly: reproduced a `403` (`x-cache: TCP_DENIED`, Akamai) curling the resolved URL from the *same machine*, seconds after resolving, with the exact headers the worker returned. This is not the URL being stale — the failure is real and immediate; the app's "URL expired" label is just the closest existing failure code for a 403, not literally accurate here.
  - **Why Seal (a popular competing downloader) doesn't hit this:** it's built on `youtubedl-android` — yt-dlp compiled to run *on the phone*, so it resolves and downloads in the same process/session, no split, session continuity holds automatically. That's exactly the mitigation already named above ("Full build only, device-side resolving with `youtubedl-android`") — this is real-world confirmation it's the right fix, not a new idea.
  - **Scope if picked up:** a new native dependency (yt-dlp-for-Android, Python runtime or compiled subprocess — needs its own §15 entry and APK-size check), Full-build-only (Lite's host allow-list already excludes third-party-platform resolvers entirely, so this doesn't apply there). Decide per-platform whether to route through the worker (most platforms, cheap, works fine) or on-device (platforms confirmed to need session continuity — TikTok today, watch for others). Logged, not started — user chose to track this and decide later rather than scope it now (2026-09-23).
- **R8 — FFmpeg adoption.** ffmpeg-kit is retired. **Decided + partially shipped 2026-09-23** (pulled forward — see §7.4): `io.github.junkfood02.youtubedl-android:ffmpeg` (Termux build, GPLv3, subprocess-only — not the originally-preferred LGPL-only static-link approach; user explicitly confirmed accepting the GPL source-offer obligation over waiting to find/vet an LGPL-only fork). Not yet checked: Android 15 16-KB page-size support, precise APK growth measurement, on-device verification of the extracted-binary path (`FfmpegRunner.kt` — couldn't be tested against a real extraction in the dev environment this was built in). MP3/OPUS conversion via this same dependency is still open, not started.
...
- FFmpeg fork/build choice — **decided 2026-09-23** (pulled forward from Phase 9, partial): `io.github.junkfood02.youtubedl-android:ffmpeg` (0.18.1, Maven Central) — same artifact Seal ships, Termux's ffmpeg build, **GPLv3** (a deliberate deviation from the earlier "LGPL-only" preference — see §7.4/R8 below and the confirmation this decision required). Invoked as a genuinely separate subprocess via `ProcessBuilder` (`FfmpegRunner.kt`), never linked into our binary — keeps our own code outside the GPL boundary but still requires offering FFmpeg's source, done via Settings > About (`AboutSection` in `screens/settings/index.tsx`). Covers only the VP9/AV1-video-mux fallback so far (`mux-ffmpeg` postProcess, stream-copy to Matroska, no re-encode) — MP3/OPUS conversion and a proper `AudioConverter` interface are still open, not started.
