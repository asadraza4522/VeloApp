...
function HistoryRow({ d, onRedownload }: { d: Download; onRedownload: () => void }) {
  ...
  const actions: { label: string; icon: React.ComponentProps<typeof MaterialCommunityIcons>["name"]; onPress: () => void }[] = [];
  if (d.status === "COMPLETED" && !missing && d.localUri) {
    const uri = d.localUri;
    const mime = d.filename?.endsWith(".m4a") ? "audio/mp4" : "video/*";
    actions.push(
      { label: "Play", icon: "play", onPress: () => router.push({ pathname: "/player/[id]", params: { id: d.id } }) },
      { label: "Open with…", icon: "open-in-new", onPress: run(() => getMedia().openInFiles(uri, mime)) },
      { label: "Share file", icon: "share-variant", onPress: run(() => getMedia().shareFile(uri, mime)) },
    );
  }
  if (missing) actions.push({ label: "Redownload", icon: "download", onPress: onRedownload });
  if (d.status === "DOWNLOADING" || d.status === "QUEUED" || d.status === "RETRYING") actions.push({ label: "Pause", icon: "pause", onPress: run(() => pauseDownload(getDeps(), d.id)) }, { label: "Cancel", icon: "close", onPress: run(() => cancelDownload(getDeps(), d.id)) });
  if (d.status === "PAUSED") actions.push({ label: "Resume", icon: "play", onPress: run(() => resumeDownload(getDeps(), d.id)) }, { label: "Cancel", icon: "close", onPress: run(() => cancelDownload(getDeps(), d.id)) });
  if (["FAILED", "CANCELED", "AUTH_REQUIRED", "SOURCE_UNAVAILABLE"].includes(d.status)) actions.push({ label: "Retry", icon: "refresh", onPress: run(() => retryDownload(getDeps(), d.id)) });
        ...
        <View style={styles.actionsRow}>
          {actions.map((a) => (
            <Pressable key={a.label} onPress={a.onPress} style={({ pressed }) => [styles.smallBtn, pressed && styles.smallBtnPressed]}>
              <MaterialCommunityIcons name={a.icon} size={15} color={colors.onSurface} />
    ...
    <ScrollView style={styles.root} contentContainerStyle={{ paddingBottom: spacing.xxxl }}>
      <Stack.Screen
        options={{
          ...
          // Not a bare tinted back arrow: it sits over the hero photo, whose top edge can be any
          // color (including near-black), so a plain tinted icon is invisible some of the time —
          // exactly the bug reported here. IconButton's opaque circle guarantees contrast no
          // matter what's in the photo. This is now the standard for any header over media.
          headerLeft: () => <IconButton icon="arrow-left" onPress={() => router.back()} label="Back" style={styles.headerBack} />,
        }}
      />
      ...
      <View style={styles.body}>
        ...
        <View style={styles.actions}>
          <Button label={history.some((d) => d.status === "COMPLETED") ? "Download again" : "Download"} icon="download" onPress={openPicker} grow />
        </View>
        <View style={styles.actions}>
          <Button label="Open source" icon="open-in-new" variant="ghost" onPress={() => void Linking.openURL(source.originalUrl)} grow />
          <Button label="Copy URL" icon="content-copy" variant="ghost" onPress={() => void Clipboard.setStringAsync(source.originalUrl)} grow />
          <Button label="Share" icon="share-variant" variant="ghost" onPress={() => void Share.share({ message: source.originalUrl })} grow />
        </View>
        ...
        <View style={styles.danger}><Button label="Delete source" icon="trash-can-outline" variant="destructive" onPress={remove} /></View>
      </View>
    </ScrollView>
  );
}
...
const useStyles = makeStyles((c) => ({
  ...
  headerBack: { marginLeft: spacing.lg },
  ...
  smallBtn: { flexDirection: "row", alignItems: "center", gap: spacing.xs, paddingHorizontal: spacing.md, paddingVertical: 6, borderRadius: radius.sm, backgroundColor: c.surfaceInset, borderWidth: 1.5, borderColor: c.border, ...tactileShadow(1, c.border) },
